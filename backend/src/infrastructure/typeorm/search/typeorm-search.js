...
export class TypeOrmSearch {
    ...
    async search(input) {
        ...
        try {
            ...
            if (options.allowedSorts && options.allowedSorts.length > 0) {
                ...
                if (!resolved) {
                    throw new DomainError(PAGINATION_ERROR.SORT_FIELD_NOT_ALLOWED, `排序白名单与列解析不一致：无法解析字段 ${sample}`);
                }
            }
            // 1) 应用文本查询
            ...
            if (error instanceof DomainError)
                throw error;
            throw new DomainError(PAGINATION_ERROR.DB_QUERY_FAILED, '搜索查询失败', { error: error instanceof Error ? error.message : '未知错误' }, error);
        }
    }
    /**
     ...
     */
    computeParams(params, defaultSorts) {
     ...
     */
    normalizeSorts(sorts, defaultSorts, cursorKey) {
     ...
     */
    applyTextSearch(qb, query, options) {
        if (!query || options.searchColumns.length === 0)
            return;
     ...
     */
    applyFilters(qb, filters, options) {
        if (!filters || !options.allowedFilters || options.allowedFilters.length === 0)
            return;
     ...
     */
    prepareSorts(normalized, options) {
        const safeSorts = whitelistSorts(normalized.sorts ?? options.defaultSorts, options.allowedSorts);
        return this.normalizeSorts(safeSorts.length ? safeSorts : options.defaultSorts, options.defaultSorts, 
     ...
     */
    applySorting(qb, orderedSorts, options) {
        orderedSorts.forEach((s, idx) => {
            const col = options.resolveColumn(s.field);
            if (!col)
                throw new DomainError(PAGINATION_ERROR.SORT_FIELD_NOT_ALLOWED, 
     ...
     * 执行 Offset 分页
     */
    async executeOffsetPagination(qb, normalized, options) {
     ...
     * 执行 Cursor 分页
     */
    async executeCursorPagination(qb, normalized, orderedSorts, options) {
        const qbTyped = qb;
        ...
        if (orderReversed) {
            orderedSorts.forEach((s, idx) => {
                const col = options.resolveColumn(s.field);
                if (!col)
                    throw new DomainError(PAGINATION_ERROR.SORT_FIELD_NOT_ALLOWED, `排序字段解析失败（白名单与列解析不一致）：${s.field}`);
        ...
        let items = (hasExtra ? rows.slice(0, normalized.limit) : rows);
        if (orderReversed) {
            items = [...items].reverse();
        }
        return {
     ...
     * 验证游标令牌与游标键配置一致
     */
    validateCursorToken(token, cursorKey) {
     ...
     * 解析游标边界使用的物理列名
     */
    resolveCursorColumns(options, cursorKey) {
     ...
     */
    buildCursorOps(orderedSorts, cursorKey) {
     ...
     * 应用游标边界到查询
     */
    applyCursorBounds(qb, token, columns, ops, edges) {
        if (edges.hasAfter) {
            qb.andWhere(`(${columns.primaryCol} ${ops.primaryOpAfter} :cursorPrimary OR (${columns.primaryCol} = :cursorPrimary AND ${columns.tieBreakerCol} ${ops.tieBreakerOpAfter} :cursorId))`, {
                cursorPrimary: token.primaryValue,
                cursorId: token.tieValue,
            });
        }
        if (edges.hasBefore) {
            qb.andWhere(`(${columns.primaryCol} ${ops.primaryOpBefore} :cursorPrimary OR (${columns.primaryCol} = :cursorPrimary AND ${columns.tieBreakerCol} ${ops.tieBreakerOpBefore} :cursorId))`, {
                cursorPrimary: token.primaryValue,
                cursorId: token.tieValue,
            });
        }
     ...
     */
    async executeCount(countQb, distinctCol) {
        if (distinctCol) {
            try {
