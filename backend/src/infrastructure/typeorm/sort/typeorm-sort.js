...
export class TypeOrmSort {
    ...
    normalizeSorts(input) {
        ...
        if (tie.primary === tie.tieBreaker) {
            throw new DomainError(PAGINATION_ERROR.INVALID_CURSOR, 'cursorKey.primary 与 tieBreaker 不可相同');
        }
        ...
        const primaryDir = finalBase.find((s) => s.field === tie.primary)?.direction;
        const fallbackDir = finalBase[0]?.direction ?? 'ASC';
        const tieDir = primaryDir ?? fallbackDir;
        const withTie = hasTie
            ? finalBase
