 ...
 * 判断查询结果是否为数组结构。
 */
function isQueryRows(value) {
    return Array.isArray(value);
}
/**
 * 执行存在性查询并返回结果。
 */
async function hasQueryResult(dataSource, query, params) {
 ...
 */
function loadMigrationDrillEnv() {
    const candidates = [
 ...
 * 解析演练数据库目标。
 */
function resolveDrillDatabaseTarget(mysqlConfig) {
    ...
    throw new Error('未指定演练数据库，请设置 DB_NAME 或 MIGRATION_DRILL_DATABASE；如确需临时建库，请设置 MIGRATION_DRILL_CREATE_TEMP_DB=true');
}
/**
 * 读取并校验 MySQL 连接配置。
 */
 ...
 */
function buildAdminDataSourceOptions(config) {
    return {
        type: 'mysql',
        host: config.host,
        port: config.port,
 ...
 */
function buildMigrationDataSourceOptions(config, databaseName) {
    return {
 ...
 * 校验关键表是否存在。
 */
async function assertRequiredTables(dataSource, databaseName) {
    for (const tableName of REQUIRED_TABLES) {
        const exists = await hasQueryResult(dataSource, `
        SELECT 1
 ...
 * 校验关键索引是否存在。
 */
async function assertRequiredIndexes(dataSource, databaseName) {
 ...
 * 校验关键外键是否存在。
 */
async function assertRequiredForeignKeys(dataSource, databaseName) {
 ...
 * 校验目标数据库名称，避免误操作生产库。
 */
function ensureSafeDatabaseTarget(databaseName) {
    ...
    if (!looksSafe && !isExplicitlyAllowed) {
        throw new Error(`数据库名 ${databaseName} 未包含 test/drill/ci，已拒绝执行。若确认安全请设置 MIGRATION_DRILL_ALLOW_NON_TEST_DB=true`);
    }
}
/**
 * 清空目标数据库中的全部表，确保 migration 在空库语义下执行。
 */
async function clearDatabaseTables(dataSource, databaseName) {
    ...
    try {
        for (const row of rows) {
            ...
            const tableNameValue = typeof tableNameCandidate === 'string' ? tableNameCandidate : String(tableNameCandidate);
 ...
 */
async function runEmptyDbMigrationDrill() {
    ...
    const migrationDataSource = new DataSource(buildMigrationDataSourceOptions(mysqlConfig, target.databaseName));
    ...
    try {
        ...
        if (target.shouldCreate) {
            try {
                await adminDataSource.query(`CREATE DATABASE ${escapeIdentifier(target.databaseName)} CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci`);
                writeInfo(`🆕 已创建空库: ${target.databaseName}`);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : '未知建库错误';
                throw new Error(`自动建库失败，请为当前账号授予 CREATE/DROP DATABASE 权限，或设置 MIGRATION_DRILL_DATABASE 使用预置空库。原始错误: ${message}`, {
                    cause: error,
                });
            }
        }
        ...
        try {
            ...
            throw new Error(`连接演练库失败，请确认账号对 ${target.databaseName} 具备访问与建表权限，或改用 MIGRATION_DRILL_DATABASE。原始错误: ${message}`, {
                cause: error,
            });
        }
        ...
        if (adminInitialized && target.shouldDrop) {
            try {
                await adminDataSource.query(`DROP DATABASE IF EXISTS ${escapeIdentifier(target.databaseName)}`);
...
void runEmptyDbMigrationDrill().catch((error) => {
