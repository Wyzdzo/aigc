...
let LoginBootstrapQueryService = (() => {
    let _classDecorators = [Injectable()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var LoginBootstrapQueryService = class {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            LoginBootstrapQueryService = _classThis = _classDescriptor.value;
        ...
        normalizeAccessGroup(accessGroup) {
            const validRoles = new Set(Object.values(IdentityTypeEnum));
            const normalized = (accessGroup ?? []).filter((role) => validRoles.has(String(role)));
            return normalized.length > 0 ? Array.from(new Set(normalized)) : [IdentityTypeEnum.REGISTRANT];
        }
    };
    return LoginBootstrapQueryService = _classThis;
})();
export { LoginBootstrapQueryService };
