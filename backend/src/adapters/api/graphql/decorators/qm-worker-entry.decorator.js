...
const QM_WORKER_ENTRY_POLICIES = {
    AI_STRICT: {
...
export function qmWorkerEntry(preset) {
    const policy = QM_WORKER_ENTRY_POLICIES[preset];
    if (preset === 'AI_STRICT') {
        return applyDecorators(SetMetadata(QM_WORKER_ENTRY_POLICY_KEY, policy), UseGuards(JwtAuthGuard, RolesGuard, QmWorkerEntryGuard), Roles(IdentityTypeEnum.STAFF, IdentityTypeEnum.ADMIN));
    }
    return applyDecorators(SetMetadata(QM_WORKER_ENTRY_POLICY_KEY, policy), UseGuards(QmWorkerEntryGuard));
}
