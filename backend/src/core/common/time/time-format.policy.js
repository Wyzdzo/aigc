...
export function formatForTimestamp3(date) {
...
export function formatForDateTime(date) {
...
function ensureNormalizedMilliseconds(value, semantic) {
    ...
    if (input.normalizedKind !== 'normalized_time' || input.semantic !== semantic) {
        return new DomainError(semantic === 'system_event_time'
            ? TIME_ERROR.INVALID_SYSTEM_EVENT_TIME
            : TIME_ERROR.INVALID_BUSINESS_DATETIME, semantic === 'system_event_time'
