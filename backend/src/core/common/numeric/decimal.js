 ...
 */
export function decimalPlaces(value, maxScale = MAX_SCALE) {
 ...
 */
export function toScaledInt(value, scale, mode = 'half-up') {
    ...
    if (!Number.isSafeInteger(result)) {
        throw new RangeError(`整数化结果超出安全整数范围 (value=${value}, scale=${scale}, mode=${mode})`);
    }
    return result;
}
/**
 ...
 */
export function safeToScaledInt(value, scale, mode = 'half-up') {
    if (!Number.isFinite(value))
        throw new RangeError('输入必须为有限的 number');
 ...
 * @param scale 固定小数位数
 */
export function fromScaledInt(scaledInt, scale) {
 ...
 * @param scale 固定小数位
 */
export function addDecimals(a, b, scale) {
 ...
 * @param scale 固定小数位
 */
export function subDecimals(a, b, scale) {
    const ai = toScaledInt(a, scale);
    const bi = toScaledInt(b, scale);
    const diff = ai - bi;
 ...
 */
export function decimalAdd(a, b, options) {
    ...
    const sum = ai + bi;
    if (!Number.isSafeInteger(sum))
        throw new RangeError(`加法结果超出安全整数范围 (scale=${scale}, mode=${mode}, ai=${ai}, bi=${bi})`);
    return fromScaledInt(sum, scale);
}
/**
 ...
 * @param options 可选参数：`scale` 自定义固定位数，`mode` 舍入模式（默认 half-up）。
 */
export function decimalSub(a, b, options) {
    const mode = options?.mode ?? 'half-up';
    const autoScale = Math.min(MAX_SCALE, Math.max(decimalPlaces(a), decimalPlaces(b)));
    const scale = options?.scale ?? autoScale;
    const ai = toScaledInt(a, scale, mode);
    ...
    const diff = ai - bi;
    if (!Number.isSafeInteger(diff))
        throw new RangeError(`减法结果超出安全整数范围 (scale=${scale}, mode=${mode}, ai=${ai}, bi=${bi})`);
    return fromScaledInt(diff, scale);
}
/**
 ...
 */
export function mulScaledInts(aInt, aScale, bInt, bScale, outScale, mode = 'half-up') {
    ...
    if (diff === 0) {
        ...
        if (!Number.isSafeInteger(asNum))
            throw new RangeError('乘法结果超出安全整数范围');
        return fromScaledInt(asNum, outScale);
    }
    // 辅助： BigInt 转安全 number
    function toSafeNumber(bi) {
        const asNum = Number(bi);
        if (!Number.isSafeInteger(asNum))
            throw new RangeError(`乘法结果超出安全整数范围 (aScale=${aScale}, bScale=${bScale}, outScale=${outScale}, diff=${diff})`);
        return asNum;
    }
    // 辅助：根据舍入模式对 BigInt 除法结果进行调整（正负数都符合数学定义）
    function roundQuotient(prod, div, rounding) {
        let q = prod / div;
 ...
 * @param mode 舍入模式，默认 half-up
 */
export function divScaledInts(aInt, aScale, bInt, bScale, outScale, mode = 'half-up') {
    if (!Number.isSafeInteger(aInt) || !Number.isSafeInteger(bInt)) {
        throw new RangeError('输入必须为安全整数');
    }
    if (bInt === 0) {
    ...
    function toSafeNumber(bi) {
        const asNum = Number(bi);
        if (!Number.isSafeInteger(asNum))
            throw new RangeError(`除法结果超出安全整数范围 (aScale=${aScale}, bScale=${bScale}, outScale=${outScale})`);
        return asNum;
    }
    function roundQuotient(dividend, divisor, rounding) {
 ...
 * @param mode 舍入模式
 */
export function mulDecimalsAuto(a, b, aScale, bScale, outScale, mode = 'half-up') {
 ...
 * @param mode 舍入模式（默认 half-up ）
 */
export function divDecimalsAuto(a, b, aScale, bScale, outScale, mode = 'half-up') {
    if (b === 0)
        throw new RangeError('除数不能为 0');
 ...
 * @param mode 舍入模式（默认 half-up ）
 */
export function mulDecimals(a, b, aScale, bScale, outScale, mode = 'half-up') {
 ...
 * @param mode 舍入模式（默认 half-up ）
 */
export function multiplyMoneyByFactor(amountYuan, factor, mode = 'half-up') {
...
export function rescaleScaledInt(params) {
    ...
    if (!Number.isSafeInteger(intValue))
        throw new RangeError(`输入必须为安全整数 (intValue=${intValue}, fromScale=${fromScale}, toScale=${toScale})`);
...
export function addIntAndDecimal(params) {
    ...
    const sum = aOp + bOp;
    if (!Number.isSafeInteger(sum))
        throw new RangeError(`加法结果超出安全整数范围 (opScale=${sOp}, outScale=${outScale}, aOp=${aOp}, bOp=${bOp})`);
 ...
 */
export function decimalCompute(params) {
    switch (params.op) {
        case 'add':
 ...
 */
function computeAdd(a, b, outScale) {
    const aNum = a;
    const bNum = b;
 ...
 * @param outScale 可选输出位，默认取运算位
 */
function computeSub(a, b, outScale) {
    const aNum = a;
    const bNum = b;
    const auto = outScale ?? Math.min(MAX_SCALE, Math.max(decimalPlaces(aNum), decimalPlaces(bNum)));
 ...
 */
function computeDiv(a, b, outScale) {
    if (b === 0)
        throw new RangeError('除数不能为 0');
 ...
 */
function computeMul(a, b, outScale) {
    const aScale = decimalPlaces(a);
    const bScale = decimalPlaces(b);
