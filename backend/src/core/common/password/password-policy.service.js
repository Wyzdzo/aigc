...
export class PasswordPolicyService {
     ...
     * 验证密码长度
     */
    validateLength(password, config) {
        const errors = [];
        let strengthBonus = 0;
     ...
     * 验证字符类型
     */
    validateCharTypes(password, config) {
        const errors = [];
        let strengthBonus = 0;
        let charTypeCount = 0;
     ...
     * 验证黑名单
     */
    validateBlacklist(password, config) {
        const errors = [];
        let maxStrength = 100;
     ...
     * @returns 改进建议
     */
    generatePasswordSuggestions(errors) {
        const suggestions = [];
